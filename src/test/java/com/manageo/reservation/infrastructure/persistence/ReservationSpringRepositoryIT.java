...
@SpringBootTest
class ReservationSpringRepositoryIT {
	
	@Autowired
	private ReservationSpringRepository repository;
	...
	@BeforeEach
	void setUp() {
		reservationEntity = ReservationEntity.from(new Reservation.ReservationBuilder().reservationId(new ReservationId(UUID.fromString(RESERVATION_ID))).restaurantId(new RestaurantId(UUID.fromString(RESTAURANT_ID)))
	...
	void shouldSaveAndGetWithRepository() {
		...
		assertThat(repository.findById(UUID.fromString(RESERVATION_ID)).get()).usingRecursiveComparison().isEqualTo(reservationEntity);
	}
